...
def build_weekly_text_report(result: AnalysisResult, week_index: int) -> WeeklyTextReport:
    ...
    start_utc = pd.Timestamp(kpis["time_utc"].min()) if not kpis.empty else pd.Timestamp(week["week_start_utc"])
    end_utc = _week_end_for_report(kpis, week, window_minutes)
    week_start = pd.Timestamp(week["week_start_utc"])
    ...
    return WeeklyTextReport(
        ...
        label=f"{date_token} to {pd.Timestamp(week['week_end_utc']).strftime('%Y-%m-%d')}",
        content="\n".join(lines) + "\n",
    )
...
def _kpis_for_week(kpis: pd.DataFrame, week: pd.Series) -> pd.DataFrame:
    if kpis.empty:
        return kpis.copy()
    start = pd.Timestamp(week["week_start_utc"])
    end = pd.Timestamp(week["week_end_utc"])
...
def _week_end_for_report(kpis: pd.DataFrame, week: pd.Series, window_minutes: int) -> pd.Timestamp:
    if kpis.empty:
        return pd.Timestamp(week["week_end_utc"])
    end = pd.Timestamp(kpis["time_utc"].max()) + pd.Timedelta(minutes=window_minutes)
    return min(end, pd.Timestamp(week["week_end_utc"]))
